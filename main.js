...
function init() {
    ...
    if (!fs.existsSync("./private.json")) throw `Please refer to the "Notice" section of README.md for more info`;
...
function build(name, ...passthrough) {
    const project = (name ?? "").toLowerCase().trim();
    if (project.length == 0) throw `Could not build project with invalid name: "${name}" (${name.length} character${name.length === 1 ? "" : "s"})`;

    const location = path.join(projects, project + "\\");
    if (!fs.existsSync(location)) throw `Could not build project with name: "${name}", project does not exist at: "${location}"`;

    const target = path.join(location, name + ".yyp");
    if (!fs.existsSync(target)) throw `Could not build project with name: "${name}", project does not contain a project file at: "${target}"`;

    const compiler = path.join(toolchain, private.Key1);
    if (!fs.existsSync(compiler)) throw `Could not resolve compiler path at: "${compiler}"`;
    
    const command = [
        `${compiler} "${target}"`,
        `--output="${path.join(location, "build")}"`,
        `--template="${path.join(toolchain, private.Key2)}"`,
        `--toolchain="${config["toolchain@path"]}"`,
        `--build-type=${config["toolchain@type"]}`,
        `--target-triple=${config["toolchain@triple"]}`,
        `--asset-compiler="${path.join(toolchain, private.Key3)}"`,
        `--asset-compiler-args="--nowad --headless --noloop"`,
        ...passthrough
    ].join(" ");
    console.log(`Building project "${name}" from the following command:\n${command}`);

    try {
        proc.execSync(command, {stdio: "inherit"}).toString();
    } catch (e) {}
    return true;
}

const version = "1.0.0", inject = undefined; //["-build", "test"];
try {
    const time = Date.now(), commands = {
        ...
        "build": {
            description: "Builds the project from Hera's workspace with the specified toolchain",
            arguments: [
                {name: "name", description: "The name of the project to build"}
            ],
            callback: build
        },
        "help": {
